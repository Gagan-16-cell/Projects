    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if bcrypt.checkpw(password.encode('utf-8'), stored_password.encode('utf-8')):
                logging.info(f"Login successful for {stored_name}")
                session['name'] = stored_name  # Store name in session
            else:
...
# QCNN Net Model and its utilities

@tf.keras.utils.register_keras_serializable(package="CustomModels")
class QuCNetEnhanced(tf.keras.Model):
    def __init__(self, **kwargs):  # Modified for config inheritance
        super().__init__(**kwargs)  # Changed to Python 3 style super()
...
def predict():
    ...
    if file and allowed_file(file.filename):
        ...
        try:
            ...
            return render_template('dashboard.html', name=session.get('name', 'User'), result=result, image_file=filename)
        except Exception as e:
...
if __name__ == '__main__':
    load_model_from_path("model/QCNN.h5")
    app.run(debug=True)
